...
public class OkHttpConnectionPoolConfig {
	...
	public Dispatcher dispatcher() {
		...
		ExecutorService executorService = new ThreadPoolExecutor(maxIdleConnections, maxRequests, keepAliveDuration, TimeUnit.SECONDS,
				new SynchronousQueue<>(), (runnable, executor) -> {

			if (!executor.isShutdown()) {

				LogUtil.HTTP.warn("all threads are busy currently in the http thread pool");
				runnable.run();
			}
		});
